...
public class ClientStateMachine extends StateMachine implements IMalmoMessageListener
{
    ...
    @Override
    protected String getName() { return "CLIENT"; }
    ...
    protected boolean areMissionsEqual(Mission m1, Mission m2)
    {
        ...
        // A simple string comparison on the serialisations won't necessarily work,
        // since there are variations in the way JAXB, CodeSynthesis etc perform the serialisation/deserialisation.
        // For the time being, we just return true here.
        return true;
    }

    /** Set up the mission poller.<br>
     * This is called during the initialisation episode, but also needs to be available
     * for other episodes in case the configuration changes, resulting in changes to the ports.
            ...
            public boolean onCommand(String command, String ipFrom, DataOutputStream dos)
            {
                ...
                else if (missionInitResult.missionInit != null)
                {
                    ...
                    if (rootNodeName != null && rootNodeName.equals("MissionInit"))
                    {
                        ...
                        {
                            // MissionInit passed to us, no server details, and role 0 specified - this means WE should become
    ...
    public abstract class ErrorAwareEpisode extends StateEpisode implements IMalmoMessageListener
    {
        ...
        protected void onAbort(Map<String, String> errorData) {}	// Default does nothing, but can be overridden.
    }

    /** Helper base class that responds to the config change and updates our AddressHelper.<br>
     * This will also reset the mission poller.
     * Depending on the state, more work may be needed (eg to recreate the command handler, etc) - it's up to the
    ...
    //---------------------------------------------------------------------------------------------------------
    /** Now the MissionInit XML has been decoded, the client needs to create the Mission Handlers.
     */
    public class CreateHandlersEpisode extends ConfigAwareStateEpisode
    {
        ...
        protected void execute() throws Exception
        {
            ...
            {
                // A specific port has been requested, and it's not the one we are currently using, so we need to recreate our poller.
            ...
            // We will either need to connect to an existing server, or to start a new integrated server ourselves, depending on our role.
    ...
    //---------------------------------------------------------------------------------------------------------
    /** Depending on the basemap provided, either begin to perform a full world load, or reset the current world
     */
    public class EvaluateWorldRequirementsEpisode extends ConfigAwareStateEpisode
    {
        ...
        protected void execute()
        {
            // We are responsible for creating the server, if required. This means we need access to the server's MissionHandlers:
            ...
            {
                // We want a new world, and there is currently a world running, so we need to kill the current world.
                episodeHasCompleted(ClientState.PAUSING_OLD_SERVER);
            }
            else if (needsNewWorld && !worldCurrentlyExists)
            {
                // We want a new world, and there is currently nothing running, so jump to world creation:
                episodeHasCompleted(ClientState.CREATING_NEW_WORLD);
            }
            else if (!needsNewWorld && worldCurrentlyExists)
            {
                // We don't want a new world, and we can use the current one - but we own the server, so we need to pass it the new mission init:
    ...
    //---------------------------------------------------------------------------------------------------------
    /** Pause the old server.
     * It's vital that we do this, otherwise it will respond to the quit disconnect package straight away and kill the server thread,
     * which means there will be no server to respond to the loadWorld code.
     * (This was the cause of the infamous "Holder Lookups" hang.)
     */
     ...
     * This state is ended by the death of the player or by the IWantToQuit handler, or by the server declaring the mission is over.
     */
    ...
    //---------------------------------------------------------------------------------------------------------
    /** State that occurs at the end of the mission, whether due to death, failure, success, error, or whatever.
     */
    public class MissionEndedEpisode extends ConfigAwareStateEpisode
    {
        ...
        private void sendMissionEnded(MissionEnded missionEnded)
        {
            ...
            if (!sentOkay)
            {
                // Couldn't formulate a reply to the agent - bit of a problem. Can't do much to alert the agent itself,
